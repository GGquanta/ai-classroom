...
![Kami 一页纸打印样例（白底变体）](/assets/html-print-document-skills/html-skill-kami-print.png)

![Kami 正式信件样例](/assets/html-print-document-skills/html-skill-kami-letter.png)
...
![HTML 优先打印流水线（手绘示意）](/assets/html-print-document-skills/html-skill-pipeline-sketch.png)
...
### 3.4 与 Markdown→DOCX 路线的边界

技术方案、标书类交付常以 Markdown 为唯一正文来源，再经脚本或 Pandoc 生成中文公文体裁 Word 文档。该路径适合内部评审与可修订协作。
...
![印刷级 Skill 角色地图（手绘示意）](/assets/html-print-document-skills/html-skill-taxonomy-sketch.png)
...
![Kami 长文 / 财报点评样例](/assets/html-print-document-skills/html-skill-kami-tesla.png)
...
![Open Design Studio 原型预览（官方资源）](/assets/html-print-document-skills/html-skill-open-design-studio.png)
...
多页咨询报告、经营分析、结构化汇报；弱于信件与作品集等 Kami 全品类。
...
![Kami 幻灯片样例（对照羊皮纸幻灯片路径）](/assets/html-print-document-skills/html-skill-kami-slides.png)
...
![场景选型速查（手绘示意）](/assets/html-print-document-skills/html-skill-selection-sketch.png)
...
| 中文标书 / 可修订公文 | Markdown→DOCX 工作流 | 不建议强行 Kami 替代 |
...
- 技术方案、标书、内部汇报：继续 Markdown → DOCX（公文体、可修订）。
- 对外印刷感 PDF、英文材料、一页纸与信件：安装 Kami 或 `doc-kami-parchment`，素材可从同一 Markdown / 纪要抽取。
- 配图：架构图可用既有绘图技能；纸面内嵌图遵循各 Skill 的单色 / 内联 SVG 约定，避免破坏羊皮纸签名。
...
可在企业内部沉淀自有“约束 Skill”：吸收第 2.5 节七项指标，注入组织或项目视觉规范（色、字体、页眉密级、免责声明），仍保持 HTML→PDF 主路径，并与现有 Markdown→DOCX 分轨。
